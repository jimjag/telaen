...
--->
        ...
        {/section}
        <tr>
          <td colspan="3" class="default">&nbsp;{$umLabel.pages_text}:&nbsp;{if $umPreviousLink}<a href="{$umPreviousLink}" class="navigation">{$umLabel.previous_text}</a> &nbsp;{/if}{$umNavBar}{if $umNextLink} &nbsp;<a href="{$umNextLink}" class="navigation">{$umLabel.next_text}</a>{/if}</td>
        </tr>
        <tr>
