...
{if $smarty.request.oefolders=="true"}
	{***** SHOWS ONLY FOLDERS *****}<body onload="javascript:parent.WriteFolders('{strip}
		...
		{if $umFolderList[i].entry eq "inbox"}<img src=themes/outlook/images/icon_folder_inbox.gif>{elseif $umFolderList[i].entry eq "sent"}<img src=themes/outlook/images/icon_folder_sent.gif>{elseif $umFolderList[i].entry eq "trash"}<img src=themes/outlook/images/icon_folder_trash.gif>{elseif $umFolderList[i].entry eq "spam"}<img src=themes/outlook/images/icon_folder_spam.gif>{else}<img src=themes/outlook/images/icon_folder_other.gif>{/if}
	</td>
	<td><nobr><font class=xx-normal></font>{assign var='unread' value=$umFolderList[i].msgs|regex_replace:"([0-9]*/)":""}<a href=\'{$umFolderList[i].chlink}\'>{if $unread!="0"}<b>{/if}
          ...
          <td valign='top' height='100%'><table width='100%' height='100%' cellpadding='0' cellspacing='0'>
              <tr>
                <td width='100%' height='100%' valign='top' class="folders_outer"><table width="100%" height="100%" cellpadding='0' cellspacing='0' class="folders_inner">
                    <tr>
                      <td colspan="6" style="background-color: #808080;padding:5;"><font class='xx-normal' color="#FFFFFF"><b>{$umLabel.folders_to} {$umUserEmail}</b></font></td>
                    </tr>
                      <tr>
                    ...
                    &nbsp;
                    <input type=button value="{$umLabel.fld_make_new}" class=button onClick="create()">
                      </td>
                      </tr>

                    <tr>
                      <td class='headers' colspan="2">{$umLabel.fld_name_hea}</td>
                      <td class='headers'>{$umLabel.fld_messages_hea}</td>
                      <td class='headers'>{$umLabel.fld_empty_hea}</td>
                      <td class='headers'>{$umLabel.fld_delete_hea}</td>
                      <td class="headers">&nbsp;</td>
                    </tr>
                    {counter start=0 skip=1 print=false}
                    {section name=i loop=$umFolderList}
