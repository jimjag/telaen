    ...
    <td valign="top"><table cellspacing="1" cellpadding="1" width="100%" border="0" bgcolor="White">
        ...
        <form name="form1" action="process.php" method="post">
          ...
          <tr>
            ...
            {/if}
            <td class="cent" nowrap> {if !$smMessageList[i].read}<b>{/if}
              {$smMessageList[i].date|date_format:$smLabel.date_format}
              {if $smMessageList[i].read eq "false"}</b>{/if} </td>
            <td class="right" nowrap> {if !$smMessageList[i].read}<b>{/if}
              {$smMessageList[i].size} &nbsp;
              {if !$smMessageList[i].read}</b>{/if} </td>
          </tr>
          {/section}
          <tr>
