...
class MyMonth {
	...
	function monthAsTable() {
		$ishtml = ($_GLOBALS['prefs']["editor-mode"] != "text");
		$ret = <<<EOT
...
EOT;
		...
		for($day=1; $day<=$this->_lastday; $day++,$weekday++){
			...
			if ($event) {
				...
				if ($day == $today)
					$dclass = "tevt";
				$event = "<div id=\"e_{$this->_year}_{$this->_month}_{$day}\" class=\"einfo\">| {$this->_mymonth['month']} {$day}, {$this->_year} |<hr/>" . ($ishtml)?$event:nl2br($event) . "</div>";
			}
